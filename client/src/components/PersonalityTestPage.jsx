...
const PersonalityTestPage = () => {
    ...
    const handleSubmit = () => {
        if (allQuestionsAnswered()) {
            ...
            // Now, you can use result.totalScore for other purposes, like saving to the backend
            saveTestResults(result.totalScore);
    ...
    const saveTestResults = async (totalScore) => {
        const token = localStorage.getItem("token");
        console.log("Frontend Token:", token);  // Log the token on the frontend
        // Determine the category based on totalScore
        let category;
        if (totalScore >= 70) {
            category = "Natural Leader";
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
                ...
                feedback: feedback,
                score: totalScore,
                category: category  // Include the category in the request body
            })
        };
    
        try {
            const response = await fetch("http://localhost:8080/api/testResultsRoutes", requestOptions);
            if (response.ok) {
    ...
    const generateFeedback = (answers) => {
        ...
        // Return the feedback
        return { feedback: feedback, totalScore: totalScore };
    };
        ...
        <>
          ...
          <Link to="/" style={{ textDecoration: 'none' }}>
            ...
            <div className={showDropdown ? styles.dropdownActive : styles.button_container}>
              ...
              <button className={styles.white_btn2} onClick={goToUserResultsPage}>
                                    Past Results
                                </button>
    ...
    <div className={styles.startButtonContainer}>
        ...
        <button className={styles.startButton} onClick={() => navigate("/myresults")}>
